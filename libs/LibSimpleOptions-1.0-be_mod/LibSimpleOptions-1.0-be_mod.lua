...
local MAJOR_VERSION = "LibSimpleOptions-1.0-be_mod"
local MINOR_VERSION = 90000 + tonumber(("$Revision: 46 $"):match("(%d+)"))
local errorMsg = {
	str="%s: bad argument '%s' (%q expected, got %q)",
	num="%s: bad argument '%s' (%q expected, got %q)",
	fnc="%s: bad argument '%s' (%q expected, got %q)"
}
...
do
	...
	local function makePanel(name, parentName, controlCreationFunc)
		...
		if not parentName then
			panel = CreateFrame("Frame", name .. "_Panel", InterfaceOptionsFramePanelContainer)
		else
			panel = CreateFrame("Frame", parentName .. "_Panel_" .. name, InterfaceOptionsFramePanelContainer)
		end
		panels[panel] = true
...
--		elseif type(args.values) ~= "function" and not tmp[args.default] then
		elseif type(args.values) ~= "function" and not args.values[args.default] then
...
--		elseif type(args.values) ~= "function" and args.current and not tmp[args.current] then
		elseif type(args.values) ~= "function" and args.current and not args.values[args.current] then
...
do
	...
	function panelMeta:MakeToggle(...)
		...
		toggle.realWidth = toggle:GetWidth()+_G[toggle:GetName() .. "Text"]:GetWidth()
...
do
	local function editbox_SetValue(self)
		self.changeFunc(self:GetText())
	end

	function panelMeta:MakeEditBox(args)
		assert( type(args.name)=="string",        errorMsg.str:format("MakeEditBox", "name",        "string",   type(args.name) ))
		assert( type(args.description)=="string", errorMsg.str:format("MakeEditBox", "description", "string",   type(args.description) ))
		if args.numeric == true then
			assert( type(args.default)=="number", errorMsg.num:format("MakeEditBox", "default",     "number",   type(args.default) ))
		else
			assert( type(args.default)=="string", errorMsg.str:format("MakeEditBox", "default",     "string",   type(args.default) ))
		end
		assert( type(args.getFunc)=="function",   errorMsg.fnc:format("MakeEditBox", "getFunc",     "function", type(args.getFunc) ))
		assert( type(args.setFunc)=="function",   errorMsg.fnc:format("MakeEditBox", "setFunc",     "function", type(args.setFunc) ))
		if args.numeric~=nil and args.passwd~=nil and args.forceNumericPasswd~=true then
			assert( args.numeric==false and args.passwd==true, "MakeEditBox: 'numeric' and 'passwd' is an undesirable combination.") -- :)
		end

		local i,name = 0,nil
		repeat
			i,name = i+1,self:GetName().."_Editbox"..i
		until not _G[name]

		local editbox = CreateFrame("Editbox",name,self,"InputBoxTemplate")
		self.controls[editbox] = true

		editbox:SetWidth(args.width~=nil and tonumber(args.width)>0 and tonumber(args.width) or 140)
		editbox:SetHeight(args.height~=nil and tonumber(args.height)>0 and tonumber(args.width) or 20)
		editbox:SetAutoFocus(args.autofocus==true)
		editbox:SetNumeric(args.numeric==true)
		editbox:SetPassword(args.passwd==true)

		editbox.label = editbox:CreateFontString(nil,"BACKGROUND","GameFontNormal")
		editbox.label:SetText(args.name)
		if args.labelLeft==true then
			editbox.label:SetPoint("TOPRIGHT",editbox,"TOPLEFT",0,0)
		else
			editbox.label:SetPoint("BOTTOMLEFT",editbox,"TOPLEFT",0,0)
		end
		editbox.ok = nil
		editbox.ok = self:MakeButton('name', "OK",'description', "",'func', function(self) editbox_SetValue(editbox) editbox.ok:Hide() end)
		editbox.ok:SetParent(editbox)
		editbox.ok:SetWidth(30)
		editbox.ok:SetHeight(20)
		editbox.ok:SetPoint("TOPRIGHT",editbox,"TOPRIGHT",0,0)
		editbox.ok:Hide()

		editbox.tooltipText = args.description
		editbox.default = args.default

		editbox.getFunc = args.getFunc
		editbox.changeFunc = args.setFunc

		local current = editbox:getFunc()
		editbox:SetText(current)
		editbox.value = current
		editbox.oldValue = current

		editbox.SetValue = editbox_SetValue
		editbox:SetScript("OnTextChanged",function(self,changed) if changed then editbox.ok:Show() end end)
		editbox:SetScript("OnEnterPressed",function(self) editbox_SetValue(self) self:ClearFocus() editbox.ok:Hide() end)
		editbox:SetScript("OnEscapePressed",function(self) editbox:SetText(editbox.oldValue) editbox.ok:Hide() self:ClearFocus() end)

		editbox.okayFunc = args.okayFunc
		editbox.cancelFunc = args.cancelFunc
		editbox.defaultFunc = args.defaultFunc

		return editbox
	end

	--function panelMeta:MakeEditMultiLineBox(...)
		-- editbox:SetMultiLine(args.multiline==true) -- multiline support coming soon...
		--local frame = CreateFrame("ScrollFrame",name,self,"InputScrollFrameTemplate")
		--frame.editbox = frame:GetChildren()[1]
		--local label = 
	--end
end
